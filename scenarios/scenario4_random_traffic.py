...
from scenarios.scenario1_burst import (
    _run_episode_with_model, _run_episode_multiagent, _run_episode_baseline,
)
...
def _make_scenario_env(seed=None):
    return PoissonTrafficEnv(num_controllers=NUM_CONTROLLERS, num_switches=NUM_SWITCHES, use_mock=True)
...
def run_rl_agent(model_path: str, n_episodes: int, is_multiagent: bool) -> Dict:
    ...
    for ep in range(n_episodes):
        ...
        logger.info(
            ...
            f"max_latency={max(result['latencies']) if result['latencies'] else 0:.3f}"
        )
...
def main(model_path: str, output_dir: str, n_episodes: int, is_multiagent: bool):
    ...
    lat_reduction = (
        (ll_result["mean_max_latency"] - rl_result["mean_max_latency"])
        / max(ll_result["mean_max_latency"], 1e-6) * 100
    )
    ...
    plot_scenario_summary(
        ...
        output_dir=output_dir, scenario_name="Poisson Traffic",
    )

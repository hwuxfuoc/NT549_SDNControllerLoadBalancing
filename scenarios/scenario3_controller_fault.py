    ...
    def _get_state_mock(self) -> np.ndarray:
        ...
        for i in range(self.num_controllers):
            ...
            if i == FAULT_CONTROLLER and self._fault_active:
                ...
                recovering = FAULT_CPU * (1 - self._recovery_progress) + base_cpu * self._recovery_progress
                load[i, 0] = np.clip(recovering + np.random.normal(0, 0.03), 0.0, 1.0)
                load[i, 1] = np.clip(recovering * 0.8 + np.random.normal(0, 0.02), 0.0, 1.0)
...
def main(model_path: str, output_dir: str, n_episodes: int, is_multiagent: bool):
    ...
    plot_scenario_summary(
        ...
        output_dir=output_dir, scenario_name="Controller Fault",
    )

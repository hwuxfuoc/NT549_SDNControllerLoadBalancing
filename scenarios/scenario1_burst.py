...
def _make_scenario_env():
    return BurstScenarioEnv(stable_steps = STABLE_STEPS, num_controllers = NUM_CONTROLLERS, num_switches = NUM_SWITCHES, use_mock = True)

def _run_episode_with_model(model, env) -> Dict:
    """Chạy 1 episode với single-agent model."""
    obs, _ = env.reset()
...
def _run_episode_multiagent(agent_models: Dict, env) -> Dict:
    """
    Chạy 1 episode với multi-agent: mỗi step dùng agent của controller bị overload nhất, chọn agent theo CPU cao nhất.
    """
    obs, _ = env.reset()
    rewards, variances, latencies = [], [], []
    done = truncated = False
...
def _run_episode_baseline(policy, env) -> Dict:
    """Chạy 1 episode với baseline dùng select_action()."""
    obs, _ = env.reset()
    if hasattr(policy, "reset"):
        policy.reset()

    # Đồng bộ switch_assignment ban đầu cho baselines có state
...
def run_rl_agent(model_path: str, n_episodes: int, is_multiagent: bool) -> Dict:
    if is_multiagent:
        ...
        for ep in range(n_episodes):
            ...
            result = _run_episode_multiagent(agent_models, env)
        ...
        for ep in range(n_episodes):
            env = _make_scenario_env()
            result = _run_episode_with_model(model, env)
            episodes.append(result)
            logger.info(f"[RL] Episode {ep+1:2d}: reward={result['total_reward']:.3f}")
...
def main(model_path: str, output_dir: str, n_episodes: int, is_multiagent: bool):
    ...
    var_reduction = (rr_result["mean_variance_cpu"] - rl_result["mean_variance_cpu"]) / max(rr_result["mean_variance_cpu"], 1e-6) * 100
    logger.info(f"\n✓ Variance CPU giảm so với Round-Robin: {var_reduction:.1f}% (mục tiêu ≥ 30%)")

    plot_scenario_summary(rewards = rl_result["rewards_per_ep"], variances = rl_result["variances"], latencies = rl_result["latencies"], migration_counts = rl_result["migrations"], output_dir = output_dir, scenario_name = "Burst Traffic")
    plot_comparison(
